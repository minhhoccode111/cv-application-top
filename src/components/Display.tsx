...
const Display: React.FC<DisplayProps> = memo(
  ({ educationItems, experienceItems, personalStates }) => {
    const { name, email, phone, address } = personalStates;
    const JSXEducation = educationItems.length !== 0 &&
      educationItems.some((item) => !item.isHidden) && (
        <>
    ...
    const JSXExperience = experienceItems.length !== 0 &&
      experienceItems.some((item) => !item.isHidden) && (
        <>
          <header className="bg-light m-2">
    ...
    return (
      <section
        id="display-cv"
        className="my-4 mx-auto max-w-4xl shadow-lg min-h-screen bg-white min-w-full"
      >
        {/* header in display section is based on personal's info */}
        <header
          className={
            'px-4 bg-darker text-light' + ' ' + (name === '' ? 'py-11' : 'py-6')
          }
        >
          <h1 className="text-center text-4xl">{name}</h1>
          <h2 className="py-2 flex items-center justify-center gap-2 text-xl">
            {email !== '' ? (
              <Icon.Email color={'#FDF0F0'} height={'24px'} width={'24px'} />
            ) : null}
            {email}
          </h2>
          <h2 className="py-2 flex items-center justify-center gap-2 text-xl">
            {phone !== '' ? (
              <Icon.Phone color={'#FDF0F0'} height={'24px'} width={'24px'} />
            ) : null}
            {phone}
          </h2>
          <h2 className="py-2 flex items-center justify-center gap-2 text-xl">
            {address !== '' ? (
              <Icon.Location color={'#FDF0F0'} height={'24px'} width={'24px'} />
            ) : null}
            {address}
          </h2>
        </header>
